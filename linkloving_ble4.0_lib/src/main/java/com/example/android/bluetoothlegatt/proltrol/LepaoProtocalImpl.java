...
public class LepaoProtocalImpl implements LepaoProtocol {
	...
	// 获取设备信息
	@Override
	public LPDeviceInfo getAllDeviceInfoNew(int userId) throws BLException, LPException {
		WatchRequset req = new WatchRequset();
		byte ff = (byte) 0xff;
		byte _00=(byte) 0x00;
		OwnLog.e("getAllDeviceInfoNew","userid"+userId);
		req.appendByte(seq++).appendByte(LepaoCommand.COMMAND_GET_ALL_USER_INFO).appendByte(ff)
				.appendInt(userId)
//				.appendByte((byte)0x0a)
//				.appendByte((byte)0x1b)
//				.appendByte((byte)0x35)
//				.appendByte((byte)0x00)
				.makeCheckSum();
		LPUtil.printData(req.getData(), " getAllDeviceInfoNew");
		WatchResponse resp = this.sendData2BLE(req);
		LPUtil.printData(resp.getData(), "0x13返回的数据");
		if(resp.getData()[4]==0){
			OwnLog.w(TAG, "0x13返回正常...");
			return resp.toDeviceInfo(LepaoCommand.COMMAND_GET_ALL_USER_INFO,"getAllDeviceInfoNew");
		}else if(resp.getData()[4]==0x01 && resp.getData()[5]==0x05){
...
//		OwnLog.i(TAG, "id："+userId);
		...
		LPUtil.printData(resp.getData(), "get registerNew");
		if(resp.getData()[4]==1)
